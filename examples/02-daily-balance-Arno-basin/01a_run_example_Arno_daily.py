"""
MOBIDIC Example - Arno River Basin

Simulation of daily water balance, year 2017-2018.
...
config_file = Path(__file__).parent / "Arno.daily.yaml"
meteodata_mat_path = (
    Path(__file__).parent.parent / "datasets" / "Arno" / "matlab" / "meteodata" / "Arno_daily_balance_2017_2018.mat"
)
...
if force_preprocessing and config.paths.meteodata:
    if not config.paths.meteodata.exists():
...
if config.paths.meteodata:
    forcing = MeteoData.from_netcdf(config.paths.meteodata)
    print("  [OK] Forcing data loaded (station-based time series).")
elif config.paths.meteoraster:
    forcing = MeteoRaster.from_netcdf(config.paths.meteoraster)
