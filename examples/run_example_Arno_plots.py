...
def main():
    ...
    fig.suptitle('MOBIDIC: Python vs MATLAB Discharge Comparison - Arno Basin',
    ...
    for i, match in enumerate(matched_reaches):
        ...
        ax_ts.plot(time_index, matlab_series, 'b-', linewidth=1.5, alpha=0.7,
                   label='MATLAB (reference)')
        ax_ts.plot(time_index, python_series, 'r--', linewidth=1.0, alpha=0.8,
                   label='Python (MOBIDICpy)')

        ax_ts.set_xlabel('Time')
        ax_ts.set_ylabel('Discharge [m³/s]')
        ax_ts.set_title(f'Reach {match["python_id"]:04d} - Time Series ' +
